...
export class CustomerEntitlementsComponent implements OnInit {
  ...
  readonly TRIAL_PROMPT_ANIMATIONS = true;
  ...
  getEntitlementRow(row: number): EntitlementEntry[] {
    let grid = this.getEntitlementGrid();
    if(grid && grid.length > row) {
      return grid[row];
    }
  }
